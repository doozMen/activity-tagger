...
extension AWContext {
    ...
    struct Enrich: AsyncParsableCommand {
        ...
        @Option(name: .long, help: "Start time (HH:MM or YYYY-MM-DD HH:MM)")
        var start: String
        
        @Option(name: .long, help: "End time (HH:MM or YYYY-MM-DD HH:MM)")
        var end: String
        ...
        func run() async throws {
            let startDate = try parseDateTime(start)
            let endDate = try parseDateTime(end)
